...
—◉ *If you believe this is an error and you have evidence, you can contact the Bot owner to appeal the suspension.*
—◉ *Contact for appeals:* wa.me/94701337346
╚═════════════════════════╝
               `.trim();
              m.reply(messageText);
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          this.reply(m.chat, `*[❗𝐈𝐍𝐅𝐎 ❗]  ${usedPrefix}buy <cantidad>*`, m);
          continue; // Limit habis
        }
        if (plugin.level > _user.level) {
          this.reply(m.chat, `*[❗𝐈𝐍𝐅𝐎 ❗] ${plugin.level}  ${_user.level}*`, m);
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;MIZTA 👑;;;\nFN:MIZTA 👑\nORG:MIZTA 👑\nTITLE:\nitem1.TEL;waid=94701337346:+94 70 133 7346 \nitem1.X-ABLabel:MIZTW 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛ.\nX-WA-BIZ-NAME:MIZTA 👑\nEND:VCARD`;
        await this.sendMessage(nk.from, {contacts: {displayName: 'MIZTA 👑', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  const msg = {
    ...
    private: '*[ ⚠️ ] NOT ADDED ..*',
    admin: '*[ ⚠️ ] NOT ADDED ..*',
    botAdmin: '*[ ⚠️ ] NOT ADDED ..*',
    unreg: '*[ 🛑 Hey!! NOT ADDED .*',
    restrict: '*[ ⚠️ ] NOT ADDED ..*',
  }[type];
  const aa = {quoted: m, userJid: conn.user.jid};
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '[ ⚠ ] MIZTY', body: 'ᴛʜᴇ MIZTY - ʙᴏᴛ', thumbnail: imagen1, sourceUrl: '..'}}}}, aa);
